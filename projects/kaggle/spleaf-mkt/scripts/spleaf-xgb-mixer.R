...
predUsingXGB <- function(train, test) {
...
trainAll = train
NUM_TRIES = 3
f_submission = matrix(rep(0,nrow(test)*NUM_TRIES), ncol=NUM_TRIES)
...
for (i in 1:NUM_TRIES) {
  ...
  train <- trainAll[sample(nrow(trainAll), min(120000, nrow(trainAll))),]
  gc()
...
#write_csv(submission, "xgb3.csv")
  pred <- predUsingXGB(train, test)
  f_submission[,i] <- pred$target
...
for (i in 1:NUM_TRIES) {
  ...
  #plot(perf)
  print (as.numeric(performance(prediction(f_submission[,i], actual[,1]), "auc")@y.values))
}

print (paste("Overall AUC:",
  as.numeric(performance(prediction(apply(f_submission,1,mean), actual[,1]), "auc")@y.values)))